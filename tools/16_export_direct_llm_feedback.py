...
def compact_stats(payload: dict | None) -> dict:
    if not isinstance(payload, dict):
        return {}
    out = {}
    for key in ("mean", "std", "min", "max"):
        if key in payload:
            out[key] = rounded(payload.get(key))
    return out
...
def visual_context(seg: dict) -> dict:
    ...
    return {
        "face_detection_rate": rounded(mediapipe.get("face_detection_rate")),
        "tongue_landmarks_available": bool(mediapipe.get("tongue_landmarks_available")),
        "mouth_opening": compact_stats(stats.get("mouth_opening")),
        "mouth_opening_ratio": compact_stats(stats.get("mouth_opening_ratio")),
        "labiodental_contact_proxy": compact_stats(stats.get("labiodental_contact_proxy")),
        "motion": {
            "mean": rounded(mouth_clip.get("motion_mean")),
            "std": rounded(mouth_clip.get("motion_std")),
            "max": rounded(mouth_clip.get("motion_max")),
        },
    }
...
def compact_segment(sample: dict, seg: dict, focus_phonemes: set[str]) -> dict:
    ...
    payload["visual_summary"] = visual_context(seg)
    return payload
...
def prompt_for_context(context: dict) -> str:
    ...
    if depth == "basic":
        extra = (
            ...
            "WavLM, or visual evidence. Explain the missing/extra sound in simple learner language."
        )
    elif depth == "none":
        ...
        extra = (
            ...
            "WavLM summary/delta, visual summary, and primary_evidence_policy."
        )
    return (
    ...
    for sample in dataset.get("samples", []):
        ...
        for seg in sample.get("segments", []):
            ...
            if not args.all_segments and interested and normalized_target not in interested:
                continue
            context = compact_segment(sample, seg, focus_phonemes)
